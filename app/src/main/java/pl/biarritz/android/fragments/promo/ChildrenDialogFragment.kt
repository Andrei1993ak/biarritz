...
import kotlinx.android.synthetic.main.fragment_children_dialog.*
...
class ChildrenDialogFragment : DialogFragment() {

    var lastVisitTime: Long? = null

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?) =
        inflater.inflate(R.layout.fragment_children_dialog, container, false)

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        val sharedPref = PreferenceManager.getDefaultSharedPreferences(context)
        lastVisitTime = sharedPref.getLong(VisitorType.CHILD.lastVisitProperty, 0)
        ...
        childrenDialog.text = text

        btnSetVisit.setOnClickListener {
            targetFragment?.let {
                if (it is OnDateRequestListener) {
                    it.onSetupVisitRequstListner(VisitorType.CHILD)
                }
            }
        }
